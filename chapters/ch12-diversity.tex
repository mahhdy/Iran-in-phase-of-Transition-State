...
\chapterheader{۱۲}{مدیریت تنوع}{همبستگی ملی در پناه رنگارنگی هویت‌ها}{EraContemp}
\label{ch:diversity}
...
\node[box=red, right=2.5cm of assim] (separ) {\textbf{تجزیه‌طلبی}\\ \scriptsize انکار وحدت\\ \scriptsize «هر قوم، یک کشور»};

% راه سوم
\node[box=bleurepublique, below=2.2cm of assim, xshift=3.2cm] (third) {\textbf{فدرالیسم همبسته}\\ \scriptsize وحدت در کثرت\\ \scriptsize «یک ایران، چند صدا»};

% فلش‌ها
\draw[->, thick, red!60] (assim) -- node[left, font=\scriptsize] {شکست} ++(0,-1.2) -| (third);
\draw[->, thick, red!60] (separ) -- node[right, font=\scriptsize] {فاجعه} ++(0,-1.2) -| (third);

% برچسب
\node[above=0.4cm of assim, xshift=3.2cm, font=\large\bfseries, color=bleurepublique] {دو افراط و یک راه میانه};
\end{tikzpicture}
\end{center}
...
\toprule
\headmark بُعد & \headmark در صورت موفقیت & \headmark در صورت شکست \\
\midrule
...
\begin{tabularx}{\textwidth}{C{1cm} R{3cm} C{3cm} C{2cm} Y}
\toprule
\headmark رد & \headmark قوم/زبان & \headmark جمعیت (م) & \headmark \% & \headmark مناطق اصلی سکونت \\
\midrule
۱ & فارس & ۴۸ & ۵۵٪ & مرکز، شرق، جنوب \\
\rowcolor{goldlight}
۲ & آذری (ترک) & ۱۷.۵ & ۲۰٪ & آذربایجان، اردبیل، زنجان \\
۳ & کرد & ۸.۷ & ۱۰٪ & کردستان، کرمانشاه، ایلام \\
\rowcolor{goldlight}
۴ & لر (بختیاری) & ۵.۲ & ۶٪ & لرستان، چهارمحال، خوزستان \\
۵ & عرب & ۲.۶ & ۳٪ & خوزستان، هرمزگان \\
\rowcolor{goldlight}
۶ & بلوچ & ۱.۷ & ۲٪ & سیستان و بلوچستان \\
۷ & ترکمن & ۰.۹ & ۱٪ & گلستان \\
\rowcolor{goldlight}
۸ & دیگران (قشقایی و...) & ۲.۵ & ۳٪ & سراسر ایران \\
\midrule
\headmark مجموع & \textbf{۸۷} & \textbf{۱۰۰٪} & \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{لبنان، سوریه و افغانستان}:
\begin{itemize}[nosep]
\item \textbf{لبنان}: تقسیم قدرت بر اساس سهمیه قومی-مذهبی (Confessionalism) که منجر به قفل‌شدگی سیاسی و ناکارآمدی دولت شد.
\item \textbf{سوریه}: انحصار قدرت در دست یک اقلیت و سرکوب دیگران که به جنگ داخلی ویرانگر و دخالت خارجی انجامید.
\item \textbf{افغانستان}: شکنندگی ساختارهای دموکراتیک در برابر وفادارهای سنتی و قومی، و عدم توازن بین مرکز و پیرامون.
\end{itemize}

\textbf{درس}: سرکوب یا تقسیم صلب قدرت (Quota-based) راهکار پایداری نیست؛ نیاز به «فدرالیسم همبسته» و «هویت ملی فراگیر» است.
\end{enghelabbox}

\subsection{همزیستی در سطح خرد: فراتر از ساختار سیاسی}

تجربه لبنان و افغانستان نشان داد که توزیع قدرت در بالا، اگر با «همگرایی اجتماعی» در پایین همراه نباشد، منجر به انجماد گسل‌ها می‌شود. پیشنهادهای تکمیلی برای همزیستی پایدار:

\begin{itemize}
    \item \textbf{بازارهای مشترک منطقه‌ای}: تشویق تجارت بین‌استان‌های با بافت قومی متفاوت برای ایجاد وابستگی متقابل اقتصادی.
    \item \textbf{مدارس میان‌قومی}: ایجاد برنامه‌های تبادل دانش‌آموز و اردوهای ملی برای شکستن پیش‌فرض‌های کلیشه‌ای قومی.
    \item \textbf{فرهنگ ملی فراگیر}: بازتعریف تاریخ و هنر ایران به گونه‌ای که تمامی اقوام خود را در آن سهیم ببینند (نه به عنوان اقلیت در کنار اکثریت، بلکه به عنوان اجزای سازنده کل واحد).
\end{itemize}
...
\begin{tikzpicture}[
    ...
    principle/.style={
        ...
        fill=blue!15,
        thick,
        minimum width=5cm, text width=5cm,
...
\node[rectangle, rounded corners=10pt, draw=green!70!black, fill=green!15,
      thick, minimum width=12cm, text width=12cm, minimum height=1.2cm, font=\large\bfseries]
...
\node[ellipse, draw=red!70, fill=red!15, thick, 
      minimum width=3.5cm, text width=3.5cm, minimum height=2cm, align=center,
...
\begin{tikzpicture}[
    level/.style={
        ...
        fill=#1!15,
        thick,
        minimum width=10cm, text width=10cm,
...
\begin{axis}[
    ...
    ybar stacked,
    bar width=25pt,
    ylabel={\rl{درصد درآمد نفت و گاز}},
    xlabel={\rl{مدل توزیع}},
    ymin=0,
    ymax=100,
    xtick={1,2,3},
    xticklabels={\rl{وضع موجود}, \rl{مدل پیشنهادی}, \rl{مدل بلندمدت}},
    legend style={
        at={(0.5,-0.2)},
        anchor=north,
        legend columns=3,
...
\node[rectangle, rounded corners=15pt, draw=blue!70, fill=blue!10,
      thick, minimum width=13cm, text width=13cm, minimum height=8cm] (umbrella) {};
\node[above=0.1cm of umbrella.north, font=\Large\bfseries, blue!70] 
    {هویت ایرانی: چتر فراگیر};

% هویت مشترک در مرکز
\node[identity=red, minimum width=4cm, text width=4cm, minimum height=2cm] at (0,1) (core) 
...
\begin{tikzpicture}[
    ...
    level/.style={
        ...
        fill=#1!20,
        thick,
        minimum width=11cm, text width=11cm,
...
%═══════════════════════════════════════════════════════════════════════════════
\section{همگرایی اجتماعی: پیوندهای نامرئی}
\label{sec:social-convergence}
%═══════════════════════════════════════════════════════════════════════════════

قانون اساسی و ساختار فدرال تنها چارچوب را می‌سازند؛ روح همزیستی در پیوندهای اجتماعی نهفته است. برای جلوگیری از «لبنانیزه شدن»، ایران باید مکانیزم‌های «پل‌زن» (Bridging) را تقویت کند:

\begin{enumerate}
    \item \textbf{شفافیت در عدالت انتقالی}: رسیدگی منصفانه به دردهای تاریخی اقوام و عذرخواهی رسمی ملی برای تبعیض‌های گذشته.
    \item \textbf{رسانه‌های کثرت‌گرا}: ترویج موسیقی، ادبیات و هنر اقوام در رسانه‌های سراسری به عنوان ثروت ملی.
    \item \textbf{احزاب فراقومی}: تشویق تشکیل احزاب سیاسی که برنامه‌های سراسری دارند و اعضای آن‌ها از تمامی مناطق و اقوام هستند.
\end{enumerate}
