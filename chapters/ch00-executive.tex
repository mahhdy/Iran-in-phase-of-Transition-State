...
\chapterheader{۰}{خلاصه مدیریتی}{نقشه راهی برای نوزایی ملی}{PrimaryDeep}
...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[
		scale=1.0, transform shape,              % ← ۲۰٪ بزرگ‌تر
		phasebox/.style={
		...
		milestone/.style={
			circle, minimum size=0.6cm, text width=0.6cm, 
		...
		\node[phasebox, fill=blue!60!black, minimum width=1.2cm, text width=1.2cm] at (0.5,1.2) {
			\begin{tabular}{c}
				\rl{فاز ۱}\\[-2pt]
				\scriptsize \rl{گذار}
			\end{tabular}
		};
		\node[phasebox, fill=green!60!black, minimum width=1.8cm, text width=1.8cm] at (2.1,1.2) {
			\begin{tabular}{c}
				\rl{فاز ۲}\\[-2pt]
				\scriptsize \rl{نهادسازی}
			\end{tabular}
		};
		\node[phasebox, fill=orange!80!black, minimum width=2.5cm, text width=2.5cm] 	at (4.5,1.2) {\rl{فاز ۳: تحکیم}};
		\node[phasebox, fill=purple!60!black, minimum width=4.0cm, text width=4.0cm] 	at (8.0,1.2) {\rl{فاز ۴: بلوغ}};
		\node[phasebox, fill=red!70!black, minimum width=2.0cm, text width=2.0cm] at (11.5,1.2) {\rl{فاز ۵: برتری}};
		\node[milestone] (y0)  at (0,0)    {};
		\node[milestone] (y2)  at (1,0) {};
		\node[milestone] (y5)  at (3,0)  {};
		\node[milestone] (y15) at (6,0)  {};
		\node[milestone] (y25) at (10,0) {};
		\node[milestone] (y30) at (12.5,0)   {};
		\node[yearlab, below=0.4cm of y0]  {\rl{۲۰۲۵}};
		\node[yearlab, below=0.4cm of y2]  {\rl{۲۰۲۷}};
		\node[yearlab, below=0.4cm of y5]  {\rl{۲۰۳۰}};
		\node[yearlab, below=0.4cm of y15] {\rl{۲۰۳۵}};
		\node[yearlab, below=0.4cm of y25] {\rl{۲۰۴۵}};
		\node[yearlab, below=0.4cm of y30] {\rl{۲۰۵۰}};
		\node[desclab, below=1cm of y0] {\rl{آغاز}\\[2pt]\rl{گذار}};
		\node[desclab, below=1cm of y2] {\rl{قانون اساسی}\\[2pt]\rl{انتخابات}};
		\node[desclab, below=1cm of y5] {\rl{نهادها}\\[2pt]\rl{استقرار}};
		\node[desclab, below=1cm of y15] {\rl{دموکراسی}\\[2pt]\rl{تثبیت}};
		\node[desclab, below=1cm of y30] {\rl{برتری}\\[2pt]\rl{منطقه‌ای}};
	\end{tikzpicture}
...
قدرت اول اقتصادی-فنی منطقه؛ الگوی جهانی؛ اقتصاد دانش‌بنیان \\
\bottomrule
\end{tabularx}
\end{table}
...
\node[font=\large\bfseries, color=red!70] at (2.5,0) {\rl{چرخه باطل}};

% چرخه فاضل (راست)
\node[cyclebox, draw=vertnapoleon, fill=bleulight] (g1) at (10,0) {
    \begin{tabular}{c}
    \rl{شفافیت و}\\
    \rl{اعتماد}
    \end{tabular}
};
\node[cyclebox, draw=vertnapoleon, fill=bleulight] (g2) at (12.5,2) {
    \begin{tabular}{c}
    \rl{مشارکت و}\\
    \rl{آزادی}
    \end{tabular}
};
\node[cyclebox, draw=vertnapoleon, fill=vertlight] (g3) at (15,0) {
    \begin{tabular}{c}
    \rl{رشد و}\\
    \rl{رفاه}
    \end{tabular}
};
\node[cyclebox, draw=vertnapoleon, fill=vertlight] (g4) at (12.5,-2) {
    \begin{tabular}{c}
    \rl{رضایت و}\\
    \rl{ثبات}
    \end{tabular}
...
\node[font=\large\bfseries, color=vertnapoleon] at (12.5,0) {\rl{چرخه فاضل}};

% فلش تبدیل
\draw[->, >=Stealth, line width=3pt, color=bleurepublique] (6,0) -- (9,0);
\node[font=\small\bfseries, color=bleurepublique, above] at (7.5,0.2) {\rl{گذار}};
...
\begin{tikzpicture}[
    ...
    node distance=1.5cm,
    timebox/.style={rectangle, rounded corners=3pt, minimum width=3.2cm, text width=3.2cm, minimum height=1.5cm, align=center, draw=bleurepublique, fill=bleulight, line width=1pt},
    arrow/.style={->, >=Stealth, thick, color=bleurepublique}
]

\node[timebox] (t1) at (0,0) {
    \begin{tabular}{c}
    \rl{\textbf{۱۰۰ روز اول}}\\[3pt]
    \rl{آرامش و معیشت}\\[2pt]
    {\scriptsize\color{vertnapoleon} \rl{بازگشت آرامش}}
    \end{tabular}
};

\node[timebox] (t2) at (4.2,0) {
    \begin{tabular}{c}
    \rl{\textbf{ماه ۶-۱۲}}\\[3pt]
    \rl{شغل، درمان، مسکن}\\[2pt]
    {\scriptsize\color{vertnapoleon} \rl{امید به آینده}}
    \end{tabular}
};

\node[timebox] (t3) at (8.4,0) {
    \begin{tabular}{c}
    \rl{\textbf{سال ۱-۳}}\\[3pt]
    \rl{زیرساخت، خدمات}\\[2pt]
    {\scriptsize\color{vertnapoleon} \rl{اعتماد به نظام}}
    \end{tabular}
};

\node[timebox] (t4) at (12.6,0) {
    \begin{tabular}{c}
    \rl{\textbf{سال ۳-۱۰}}\\[3pt]
    \rl{توسعه پایدار}\\[2pt]
    {\scriptsize\color{vertnapoleon} \rl{تثبیت دموکراسی}}
    \end{tabular}
};
...
\begin{axis}[
    ...
    xticklabels={\rl{فاز ۱}, \rl{فاز ۲}, \rl{فاز ۳}, \rl{فاز ۴}, \rl{فاز ۵}},
    xtick=data,
    ymin=0,
    ymax=220,
    bar width=20pt,
...
\addplot[fill=goldphoenix, draw=goldphoenix!70!black] coordinates {(P1,15) (P2,0) (P3,0) (P4,0) (P5,0)};
\addplot[fill=bleurepublique, draw=bleurepublique!70!white] coordinates {(P1,35) (P2,0) (P3,0) (P4,0) (P5,0)};
...
\vspace{0.3cm}
{\large\bfseries آینده ساختنی است، نه به انتظار نشستنی!.}
\vspace{0.3cm}
