...
\chapterheader{۱۰}{فاز ۳: تحکیم}{جایی که دموکراسی تنها بازی شهر می‌شود}{EraModern}
\label{chap:phase3}
...
% مرکز
\node[circle, minimum size=3cm, text width=3cm, draw=goldphoenix, fill=goldphoenix, text=white] (center) at (0,0) {\rl{برنامه ملی آب}};

% محورها
\node[action] (agri) at (-4,2.5) {\rl{کشاورزی هوشمند}\\ \tiny \rl{کاهش ۴۰٪ مصرف}};
\node[action] (urban) at (4,2.5) {\rl{مدیریت شهری}\\ \tiny \rl{بازیافت ۱۰۰٪ فاضلاب}};
\node[action] (ind) at (-4,-2.5) {\rl{بخش صنعت}\\ \tiny \rl{بازچرخانی داخلی}};
\node[action] (env) at (4,-2.5) {\rl{احیای تالاب‌ها}\\ \tiny \rl{اولویت دریاچه ارومیه}};
...
% ایران در مرکز
\node[country=blue, minimum width=3cm, text width=3cm, minimum height=2cm] (iran) at (0,0) {ایران};
...
% مرکز
\node[ellipse, draw=purple!70, fill=purple!10, minimum width=4cm, text width=4cm, 
...
% ورودی
\node[achieve=gray, minimum width=4cm, text width=4cm] (input) at (0,0) {
    \begin{tabular}{c}
    ورودی از فاز ۲:\\
    نهادهای نوپا
    \end{tabular}
...
% خروجی
\node[achieve=red, minimum width=4.5cm, text width=4.5cm, minimum height=2cm] (output) at (15,0) {
    \begin{tabular}{c}
    خروجی به فاز ۴:\\
