...
\chapterheader{۱۵}{پایش و ارزیابی}{هوشیاری مستمر در مسیر تعالی}{NeutralDark}
\label{ch:monitoring}
...
\begin{tabularx}{\textwidth}{C{0.5cm} Y C{0.8cm} C{0.8cm} C{0.8cm} C{0.8cm} C{0.8cm}}
\toprule
\headmark ردیف & \headmark شاخص پایش دموکراتیک & \headmark مبدأ & \headmark س۲ & \headmark س۵ & \headmark س۱۰ & \headmark س۱۵ \\
\midrule
...
% نهادهای رسمی
\node[box] (stats) at (0,4) {\rl{مرکز ملی آمار}\\ \tiny \rl{جمع‌آوری و صحت‌سنجی}};
\node[box] (eval) at (0,2) {\rl{سازمان پایش و ارزیابی}\\ \tiny \rl{تحلیل استراتژیک}};
\node[box] (audit) at (0,0) {\rl{دیوان محاسبات}\\ \tiny \rl{ممیزی و تطبیق}};

% نظارت مدنی
\node[box, draw=goldphoenix, fill=goldlight] (civil) at (5,4) {\rl{رصدخانه‌های مردمی}\\ \tiny \rl{پایش از پایین به بالا}};
\node[box, draw=goldphoenix, fill=goldlight] (media) at (5,2) {\rl{رسانه‌های آزاد}\\ \tiny \rl{رکن چهارم نظارت}};
\node[box, draw=goldphoenix, fill=goldlight] (intl) at (5,0) {\rl{ناظران بین‌المللی}\\ \tiny \rl{مطابقت با استانداردها}};
...
% مراحل چرخه اصلاح
\node[box] (detect) {\rl{شناسایی}\\ \tiny \rl{انحراف از هدف}};
\node[box, right=0.8cm of detect] (analyze) {\rl{علت‌یابی}\\ \tiny \rl{تحلیل ریشه‌ای}};
\node[box, right=0.8cm of analyze] (design) {\rl{طراحی}\\ \tiny \rl{تدوین راه‌حل}};
\node[box, below=1.2cm of design, draw=goldphoenix, fill=goldlight] (decide) {\rl{تصمیم‌گیری}\\ \tiny \rl{تصویب در کابینه}};
\node[box, left=0.8cm of decide] (implement) {\rl{اجرا}\\ \tiny \rl{پیاده‌سازی اصلاح}};
\node[box, left=0.8cm of implement] (monitor) {\rl{پایش مجدد}\\ \tiny \rl{ارزیابی اثربخشی}};
...
\begin{tikzpicture}[
    ...
    actor/.style={
        ...
        fill=bleulight,
        line width=1.5pt,
        minimum size=2cm, text width=2cm,
        align=center,
        font=\tiny\bfseries
    }
]
% مرکز - دولت
\node[actor, minimum size=2.8cm, text width=2.8cm, draw=goldphoenix, fill=goldlight] (gov) {\rl{دولت نوین}\\ \tiny \rl{پاسخگوی اصلی}};

% حلقه‌های پاسخگویی
\node[actor, above=2cm of gov] (parl) {\rl{مجلس}\\ \tiny \rl{نظارت سیاسی}};
\node[actor, right=2.5cm of gov] (audit) {\rl{دیوان محاسبات}\\ \tiny \rl{نظارت مالی}};
\node[actor, below=2cm of gov] (civil) {\rl{جامعه مدنی}\\ \tiny \rl{نظارت مردمی}};
\node[actor, left=2.5cm of gov] (media) {\rl{رسانه‌ها}\\ \tiny \rl{نظارت افکار عمومی}};
...
\foreach \ang/\label/\name in {45/قضا/court, 135/آمبودزمان/omb, 225/احزاب/party, 315/بین‌الملل/intl} {
    \node[actor, minimum size=1.8cm, text width=1.8cm] (\name) at (\ang:4.5) {\rl{\label}};
