...
\node[rectangle, rounded corners=10pt, draw=green!70!black, fill=green!15, 
      thick, minimum width=12cm, minimum height=1.2cm] (title) 
    {\large\textbf{چشم‌انداز فاز چهارم: ایران بالغ دموکراتیک}};

% ردیف اول
\node[box, below=1.5cm of title, xshift=-4cm] (n1) 
    {\textbf{نهادها}\\ \scriptsize پختگی و کارآمدی};
\node[box, below=1.5cm of title] (n2) 
    {\textbf{فرهنگ}\\ \scriptsize درونی‌سازی ارزش‌ها};
\node[box, below=1.5cm of title, xshift=4cm] (n3) 
    {\textbf{اقتصاد}\\ \scriptsize دانش‌بنیان و متنوع};

% ردیف دوم
\node[box, below=1.5cm of n1, xshift=2cm, fill=yellow!15, draw=yellow!70!black] (n4) 
    {\textbf{منطقه}\\ \scriptsize قدرت متوسط مؤثر};
\node[box, below=1.5cm of n3, xshift=-2cm, fill=yellow!15, draw=yellow!70!black] (n5) 
    {\textbf{جهان}\\ \scriptsize عضو محترم جامعه بین‌الملل};

% هدف نهایی
\node[rectangle, rounded corners=10pt, draw=red!70, fill=red!10, 
      thick, minimum width=8cm, minimum height=1cm, below=1.5cm of n4, xshift=2cm] (goal) 
    {\textbf{هدف}: دموکراسی تثبیت‌شده و برگشت‌ناپذیر};
...
\begin{axis}[
    ...
    ybar stacked,
    bar width=20pt,
    ylabel={درصد تولید ناخالص داخلی},
    xlabel={سال},
    ymin=0,
    ymax=100,
    xtick={1,2,3,4,5},
    xticklabels={۱۴۰۳ (مبدأ), سال ۵, سال ۱۰, سال ۱۵, سال ۲۵},
    legend style={
        at={(0.5,-0.2)},
        anchor=north,
        legend columns=3,
...
% نفت و گاز
\addplot[fill=gray!70, draw=gray!90] coordinates {(1,35) (2,28) (3,20) (4,15) (5,10)};
% صنعت سنتی
\addplot[fill=blue!50, draw=blue!70] coordinates {(1,25) (2,24) (3,22) (4,20) (5,18)};
% خدمات
\addplot[fill=green!50, draw=green!70] coordinates {(1,30) (2,32) (3,35) (4,37) (5,38)};
% دانش‌بنیان
\addplot[fill=orange!60, draw=orange!80] coordinates {(1,5) (2,10) (3,15) (4,20) (5,28)};
% گردشگری
\addplot[fill=purple!50, draw=purple!70] coordinates {(1,5) (2,6) (3,8) (4,8) (5,6)};

\legend{نفت و گاز, صنعت سنتی, خدمات, دانش‌بنیان و فناوری, گردشگری}
\end{axis}
\end{tikzpicture}
\captionof{figure}{تحول ساختار اقتصادی ایران در افق ۲۵ ساله}
\label{fig:economic-transformation}
\end{center}
...
% ردیف اول - اولویت یک
\node[priority=red] (ai) {\textbf{هوش مصنوعی}\\ \scriptsize پردازش زبان فارسی};
\node[priority=red, left=0.5cm of ai] (bio) {\textbf{زیست‌فناوری}\\ \scriptsize دارو و تجهیزات};
\node[priority=red, right=0.5cm of ai] (energy) {\textbf{انرژی پاک}\\ \scriptsize خورشیدی و هیدروژن};

% ردیف دوم - اولویت دو
\node[priority=blue, below=1cm of ai] (water) {\textbf{فناوری آب}\\ \scriptsize شیرین‌سازی و بازیافت};
\node[priority=blue, left=0.5cm of water] (space) {\textbf{فضایی}\\ \scriptsize ماهواره و سنجش};
\node[priority=blue, right=0.5cm of water] (nano) {\textbf{نانوفناوری}\\ \scriptsize مواد پیشرفته};

% ردیف سوم - اولویت سه
\node[priority=green, below=1cm of water] (fintech) {\textbf{فین‌تک}\\ \scriptsize بانکداری دیجیتال};
\node[priority=green, left=0.5cm of fintech] (agri) {\textbf{کشاورزی هوشمند}\\ \scriptsize کم‌آب‌بر};
\node[priority=green, right=0.5cm of fintech] (health) {\textbf{سلامت دیجیتال}\\ \scriptsize پزشکی از راه دور};

% برچسب‌ها
\node[left=1.5cm of bio, font=\footnotesize\bfseries, text=red!70] {اولویت اول};
\node[left=1.5cm of space, font=\footnotesize\bfseries, text=blue!70] {اولویت دوم};
\node[left=1.5cm of agri, font=\footnotesize\bfseries, text=green!70!black] {اولویت سوم};
\end{tikzpicture}
\end{center}
...
% مرکز
\node[circle_node=blue, minimum size=3cm] (iran) {\textbf{ایران}\\ قدرت متوسط\\ مسئول};

% اطراف
\node[circle_node=green, above=1.5cm of iran] (eco) {\textbf{اقتصادی}\\ هاب تجاری};
\node[circle_node=orange, below=1.5cm of iran] (sec) {\textbf{امنیتی}\\ ثبات‌بخش};
\node[circle_node=purple, left=1.5cm of iran] (cul) {\textbf{فرهنگی}\\ تمدن‌ساز};
\node[circle_node=red, right=1.5cm of iran] (dip) {\textbf{دیپلماتیک}\\ میانجی};

% خطوط اتصال
\draw[thick, blue!50, dashed] (iran) -- (eco);
\draw[thick, blue!50, dashed] (iran) -- (sec);
\draw[thick, blue!50, dashed] (iran) -- (cul);
\draw[thick, blue!50, dashed] (iran) -- (dip);
\end{tikzpicture}
\end{center}
...
\node[rectangle, rounded corners=15pt, draw=green!70!black, fill=green!5,
      thick, minimum width=14cm, minimum height=10cm] (main) {};
...
\node[rectangle, rounded corners=10pt, draw=yellow!70!black, fill=yellow!20,
      thick, minimum width=10cm, minimum height=1.2cm, align=center]
...
% عرضه آب
\addplot[color=blue, mark=*, thick, mark size=3pt, line width=1.5pt] coordinates {
    (0, 90) (5, 88) (10, 92) (15, 100) (20, 105) (25, 110)
};
% تقاضای آب (سناریوی اصلاح‌شده)
\addplot[color=green!70!black, mark=square*, thick, mark size=3pt, line width=1.5pt] coordinates {
    (0, 105) (5, 98) (10, 90) (15, 85) (20, 82) (25, 80)
};
% تقاضای آب (بدون اصلاح)
\addplot[color=red, mark=triangle*, dashed, thick, mark size=3pt] coordinates {
    (0, 105) (5, 112) (10, 120) (15, 128) (20, 135) (25, 142)
};

\legend{عرضه پایدار آب, تقاضا (با مدیریت), تقاضا (بدون مدیریت)}

% ناحیه تعادل
\fill[green!20, opacity=0.3] (axis cs:15,80) rectangle (axis cs:25,110);
\node[font=\small] at (axis cs:20,95) {ناحیه تعادل};
\end{axis}
\end{tikzpicture}
\captionof{figure}{مسیر دستیابی به تعادل آبی تا سال ۲۵}
\label{fig:water-balance}
\end{center}
...
\node[rectangle, rounded corners=10pt, draw=blue!70, fill=blue!20, 
      thick, minimum width=3cm, minimum height=1.5cm] (iran) 
    {\textbf{ایران ۱۴۲۹}\\ قدرت متوسط جهانی};

% سازمان‌ها
\node[org=green, above left=2cm of iran] (un) {سازمان ملل\\ \scriptsize عضو فعال شورای امنیت؟};
\node[org=orange, above right=2cm of iran] (wto) {WTO\\ \scriptsize عضو کامل};
\node[org=purple, below left=2cm of iran] (oecd) {OECD\\ \scriptsize در مسیر عضویت};
\node[org=teal, below right=2cm of iran] (sco) {شانگهای\\ \scriptsize عضو اصلی};
\node[org=red, left=3cm of iran] (gulf) {پیمان خلیج فارس\\ \scriptsize بنیان‌گذار};
\node[org=darkyellow, right=3cm of iran] (ecco) {ECO\\ \scriptsize رهبری منطقه‌ای};

% اتصالات
\draw[thick, blue!50] (iran) -- (un);
\draw[thick, blue!50] (iran) -- (wto);
\draw[thick, blue!50] (iran) -- (oecd);
\draw[thick, blue!50] (iran) -- (sco);
\draw[thick, blue!50] (iran) -- (gulf);
\draw[thick, blue!50] (iran) -- (ecco);
\end{tikzpicture}
\captionof{figure}{جایگاه ایران در سازمان‌های بین‌المللی تا سال ۲۵}
\label{fig:iran-international}
\end{center}
...
\begin{tikzpicture}
% کادر اصلی
\node[rectangle, rounded corners=10pt, draw=gray!70, fill=gray!5,
      thick, minimum width=14cm, minimum height=11cm] (dashboard) {};

% عنوان
\node[above=0.2cm of dashboard.north, font=\large\bfseries] 
    {داشبورد ملی پایش پیشرفت — شاخص‌های کلیدی};
...
\end{tikzpicture}
\captionof{figure}{داشبورد ۹ شاخص کلیدی پیشرفت ملی}
\label{fig:national-dashboard}
\end{center}
...
\begin{center}
\begin{tikzpicture}
\node[rectangle, rounded corners=15pt, draw=blue!70, fill=blue!5,
      thick, minimum width=12cm, minimum height=4cm, align=center] {
    {\Large\textbf{ایران ۱۴۲۹: یک رؤیای دست‌یافتنی}}\\[0.5cm]
    {\large آباد | آزاد | سربلند}\\[0.3cm]
    {\normalsize «وطن ما، مسئولیت ما، آینده ما»}
};
\end{tikzpicture}
\end{center}
